...
for i in range(m,n):
    plt.plot(x[x0:xn], Full[i][26][x0:xn], label='goal', color='#E6C200', linewidth = s, alpha = a)
plt.ylabel(r"$P_1(g_1)$")
plt.xlabel("sensor length")
plt.show()

for i in range(m,n):
    plt.plot(x[x0:xn], NoMorph[i][26][x0:xn], label='goal', color='#E6C200', linewidth = s, alpha = a)
plt.ylabel(r"$P_2(g_1)$")
plt.xlabel("sensor length")
plt.show()

for i in range(m,n):
    plt.plot(x[0:10], NoCaus[i][26][0:10], label='goal', color='#E6C200', linewidth = s, alpha = a)
plt.ylabel(r"$P_3(g_1)$")
plt.xlabel("sensor length")
plt.show()
...
for i in range(m,n):
     ...
     ax3.plot(x[x0:xn], np.add(Full[i][5][x0:xn], Full[i][18][x0:xn]), label='intinf', color='black',  linewidth = s, alpha = a)
